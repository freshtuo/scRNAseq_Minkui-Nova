...
workdir <- paste(basedir, "DropSeq_MJ_171005" )
cellcyclegenes <- "D:\\Tools\\Seurat\\CellCycles.v2\\regev_lab_cell_cycle_genes.uppercase.txt"
countfile <- file.path(basedir, "counts.merged.clean.with_MT.no_zero_counts_genes.txt.gz")
infodir<- file.path(basedir, "Info")
SingleRinfodir <- file.path(basedir, "SingleRInfo")
...
coordfile.fromSPRING <- file.path(basedir,"Spring", "SPRING.coordinates4678.txt")
colorfile.fromPaper <- paste(infodir, "cell.colors.txt" )
orderfile.fromPaper <- paste(infodir, "cell.plotting.order.txt" )
SpringPlots<-file.path(basedir,"SpringPlots")
normscaler <- 1e4
rseed <- 98
project <- "Ming"
...
ggsave(file.path(SpringPlots,"SPRING.filtered.by_condition.png"), width=11, height=8, dpi=300)
ggsave(file.path(SpringPlots,"SPRING.filtered.by_condition.svg"), width=11, height=8, dpi=300)
...
g <- g + theme_classic()
g <- g + theme(axis.title=element_blank(), axis.text=element_blank(), axis.line=element_blank(), axis.ticks=element_blank())
ggsave(file.path(SpringPlots,"SPRING.filtered.by_condition.separated.png"), width=21, height=16, dpi=300)
ggsave(file.path(SpringPlots,"SPRING.filtered.by_condition.separated.svg"), width=21, height=16, dpi=300)
...
g <- g + theme_classic()
ggsave(file.path(SpringPlots,"SPRING.filtered.exp_PRMT1.png"), width=10, height=8, dpi=300)
...
g <- g + scale_color_gradient(low="gray", high="red")
g <- g + theme_classic()
ggsave(file.path(SpringPlots,"SPRING.filtered.exp_PRMT1.separated_conditions.png"), width=20, height=16, dpi=300)
...
g <- g + stat_summary(fun.data=mean_sdl, geom="pointrange", color="gray75")
g <- g + theme_classic()
ggsave(file.path(figdir, "violin.exp_PRMT1.origin.all_conditions.per_group.png"), width=8, height=6, dpi=300)
...
g <- g + stat_summary(fun.data=mean_sdl, geom="pointrange", color="gray75")
g <- g + theme_classic()
ggsave(file.path(figdir, "violin.exp_PRMT1.origin.all_conditions.minkui.png"), width=8, height=6, dpi=300)
...
g <- g + scale_color_gradient(low="gray", high="red")
g <- g + theme_classic()
ggsave(file.path(SpringPlots,"SPRING.filtered.exp_PRMT1.orig.D8_stimulated.png"), width=10, height=8, dpi=300)
...
g <- g + stat_summary(fun.data=mean_sdl, geom="pointrange", color="gray75")
g <- g + theme_classic()
ggsave(file.path(figdir, "violin.exp_PRMT1.origin.D8_stimulated.per_group.png"), width=8, height=6, dpi=300)
...
# g <- g + theme_classic()
# ggsave(file.path(figdir, "histogram.nCount_RNA.per.cell.png"), width=8, height=6, dpi=300)
# 
# # use all cells
...
# g <- g + stat_summary(fun.data=mean_sdl, geom="pointrange", color="gray75")
# g <- g + theme_classic()
# ggsave(paste(figdir, "violin.exp_PRMT1.magic.D8_stimulated.per_group.png" ), width=8, height=6, dpi=300)
...
# g <- g + stat_summary(fun.data=mean_sdl, geom="pointrange", color="gray75")
# g <- g + theme_classic()
# ggsave(paste(figdir, "violin.exp_DUSP4.magic.D8_stimulated.per_group.png" ), width=8, height=6, dpi=300)
# # mean expression per group
...
# g <- g + stat_summary(fun.data=mean_sdl, geom="pointrange", color="gray75")
# g <- g + theme_classic()
# ggsave(paste(figdir, "violin.exp_ACTB.magic.D8_stimulated.per_group.png" ), width=8, height=6, dpi=300)
# # mean expression per group
...
# g <- g + stat_summary(fun.data=mean_sdl, geom="pointrange", color="gray75")
# g <- g + theme_classic()
# ggsave(paste(figdir, "violin.exp_GAPDH.magic.D8_stimulated.per_group.png" ), width=8, height=6, dpi=300)
# # mean expression per group
...
g <- g + theme_classic()
ggsave(file.path(SpringPlots, "locate.CD34.CD38.positive.cells.png"), width=8, height=6, dpi=300)
...
ggsave(file.path(figdir, "violin.expression.CD38.png"), width=4, height=6, dpi=300)
...
g <- g + geom_jitter(shape=16, position=position_jitter(0.2), alpha=0.3)
g <- g + stat_summary(fun.data=median_hilow, geom="pointrange", color="gray75")
ggsave(file.path(figdir, "violin.expression.CD38.target.png"), width=4, height=6, dpi=300)
...
#g <- g + scale_color_manual(values=c("yes"="red4","no"="royalblue1"))
g <- g + theme_classic()
ggsave(file.path(SpringPlots, "locate.CD34.CD38.positive.restrict.cells.png"), width=8, height=6, dpi=300)
...
singler.cellann.file <- file.path(infodir,"annotations.for.SingleR.txt.gz")
...
ggsave(filename=paste(figdir,"singler.SPRING.HPCA.main_cell_type.png" ), plot=out$p, width=10, height=8, units="in", dpi=300)

# manually plot SPRING - cell type
g <- ggplot(out$df, aes(x=x, y=y, color=ident))
g <- g + geom_point(shape=19, size=2, alpha=0.6)
g <- g + theme_classic()
ggsave(filename=paste(figdir,"singler.SPRING.HPCA.main_cell_type.manual.png" ), width=10, height=8, units="in", dpi=300)
g <- g + facet_wrap(~ident, ncol=4)
ggsave(filename=paste(figdir,"singler.SPRING.HPCA.main_cell_type.manual.split.png" ), width=20, height=16, units="in", dpi=300)
...
my.ct <- AddModuleScore(object=matt, features=genes.list, name=enrich.name, seed=rseed, ctrl=min(vapply(X = genes.list, FUN = length, FUN.VALUE = numeric(1))))
...
save(matt, file=paste(infodir, "matt.celltype.Robj" ))
...
# 4678   9
write.table(spring.n.celltype, file=paste(infodir, "info.table.celltype.txt" ), quote=FALSE, na="", sep="\t", col.names=NA)
...
ggsave(paste(figdir, "SPRING.Plot.by_cell_type.png" ), width=11, height=8, dpi=300)
g <- g + facet_wrap(~CellType, ncol=3)
ggsave(paste(figdir, "SPRING.Plot.by_cell_type.separated.png" ), width=22, height=16, dpi=300)
...
g <- g + geom_jitter(shape=16, position=position_jitter(0.2), alpha=0.3)
g <- g + stat_summary(fun.data=median_hilow, geom="pointrange", color="gray75")
ggsave(paste(figdir, "Violin.celltype.scores.png" ), width=8, height=6, dpi=600)
...
g <- g + geom_point(shape=19, size=2, alpha=0.6)
g <- g + theme_classic()
ggsave(paste(figdir, "SPRING.top.ba.cells.png" ), width=10, height=8, dpi=600)
#--------------------------------------------------------------------------------------------------------------------#
...
my.cell.type.forPlot.sorted <-my.cell.type.forPlot[with(my.cell.type.forPlot, order(cohort)),]
# plot
# my.cell.type.forPlot does not exist
...
ggsave(file.path(figdir, "SPRING.cell.types.manual.by.markers.png" ), width=10, height=8, dpi=600)
g <- g + facet_wrap(~cohort, ncol=3)
ggsave(file.path(figdir, "SPRING.cell.types.manual.by.markers.separated.png" ), width=15, height=8, dpi=600)

# rename cell type
rename.cell.type <- c("Others", "Ba", "GN", "Ly", "M", "Mk")
names(rename.cell.type) <- c("Unknown", "Ba", "GN", "Ly", "M", "Meg")
my.cell.type.forPlot$cohort.rename <- rename.cell.type[my.cell.type.forPlot$cohort]

# add PRMT1 expression
my.cell.type.forPlot.sorted.exp <- merge(my.cell.type.forPlot, cell.info.merged[,c("nFeature_RNA","nCount_RNA","orig.ident","percent.mito","percent.ribo","PRMT1")], by=0, all=T)
...
ggsave(file.path(figdir, "SPRING.cell.types.manual.by.markers.v2.png" ), width=10, height=8, dpi=600)
ggsave(file.path(figdir, "SPRING.cell.types.manual.by.markers.v2.svg" ), width=10, height=8)
g <- g + facet_wrap(~orig.ident, ncol=2)
ggsave(file.path(figdir, "SPRING.cell.types.manual.by.markers.v2.split.png" ), width=10, height=8, dpi=600)
ggsave(file.path(figdir, "SPRING.cell.types.manual.by.markers.v2.split.svg" ), width=10, height=8)
...
g <- g + theme(legend.title=element_blank(), legend.text=element_text(size=18))
ggsave(file.path(figdir, "SPRING.exp_PRMT1.v2.png" ), width=10, height=8, dpi=600)
ggsave(file.path(figdir, "SPRING.exp_PRMT1.v2.svg" ), width=10, height=8)
g <- g + facet_wrap(~orig.ident, ncol=2)
ggsave(file.path(figdir, "SPRING.exp_PRMT1.v2.split.png" ), width=10, height=8, dpi=600)
ggsave(file.path(figdir, "SPRING.exp_PRMT1.v2.split.svg" ), width=10, height=8)

# save cell type results
write.table(my.cell.type.forPlot.sorted.exp, file=file.path(infodir, "info.table.manual_celltype_all.txt" ), quote=FALSE, na="", sep="\t", col.names=NA)
...
for(tgene in c('ITGA2B','GP1BA','PF4','IFNAR1')){
  ...
  g <- g + scale_color_gradient(low="lightgray", high="red4")
  g <- g + theme(legend.title=element_blank(), legend.text=element_text(size=18))
  ggsave(file.path(SpringPlots, paste("SPRING.exp_",tgene,"v2.png",sep='.') ), width=10, height=8, dpi=600)
  ggsave(file.path(SpringPlots, paste("SPRING.exp_",tgene,"v2.svg",sep='.') ), width=10, height=8)
  g <- g + facet_wrap(~orig.ident, ncol=2)
  ggsave(file.path(SpringPlots, paste("SPRING.exp_",tgene,"v2.split.png",sep='.') ), width=10, height=8, dpi=600)
  ggsave(file.path(SpringPlots, paste("SPRING.exp_",tgene,"v2.split.svg",sep='.') ), width=10, height=8)
}
...
g <- g + theme_classic()
g <- g + scale_color_manual(values=my.cols.v2)
ggsave(file.path(SpringPlots, "SPRING.cell.types.manual.by.n-1.markers.png" ), width=10, height=8, dpi=600)
g <- g + facet_wrap(~cohort, ncol=3)
ggsave(file.path(SpringPlots, "SPRING.cell.types.manual.by.n-1.markers.separated.png" ), width=15, height=8, dpi=600)
#--------------------------------------------------------------------------------------------------------------------#
...
ggsave(file.path(SpringPlots, "SPRING.cell.types.manual.by.markers.v3.png" ), width=10, height=8, dpi=600)
g <- g + facet_wrap(~cohort, ncol=3)
ggsave(file.path(SpringPlots, "SPRING.cell.types.manual.by.markers.v3.separated.png" ), width=15, height=8, dpi=600)
#--------------------------------------------------------------------------------------------------------------------#
...
my.scatter.plot <- function(geneA, geneB, expMat, tlabel=NULL){
  ...
  toutfile <- file.path(figdir, paste("scatter","plot","exp",geneA,geneB,"png",sep=".") )
  if (! is.null(tlabel)){
    toutfile <- file.path(figdir, paste("scatter","plot","exp",geneA,geneB,tlabel,"png",sep=".") )
  }
...
g <- g + geom_vline(xintercept=1000, color="red")
g <- g + theme_classic()
ggsave(file.path(figdir, "histogram.nCount_RNA.per.cell.D8_CD42bCD41.png" ), width=8, height=6, dpi=300)
...
g <- g + scale_color_viridis_c(option="B")
g <- g + theme_classic()
ggsave(file.path(figdir,"exp.PRMT1.before_magic.D8_CD42bCD41.png",sep="\\"), width=8, height=6, dpi=300)
# after
g <- ggplot(tcounts.magic) + geom_point(aes(x=VIM, y=GAPDH, color=PRMT1))
g <- g + scale_color_viridis_c(option="B")
g <- g + theme_classic()
ggsave(file.path(figdir,"exp.PRMT1.after_magic.D8_CD42bCD41.png",sep="\\"), width=8, height=6, dpi=300)
...
dim(texp.magic)
# 10656  1077
write.table(texp.magic, file=gzfile(file.path(infodir,"exp.sqrt.after_magic.D8_CD42bCD41.tsv.gz",sep="\\")), quote=FALSE, na="", sep="\t", col.names=NA)
###write.table(t(as.matrix((counts.magic.all$result)^2)), file=gzfile(paste(infodir,"exp.pseudo_counts.after_magic.tsv.gz",sep="/")), quote=FALSE, na="", sep="\t", col.names=NA)
...
# save magic object
save(tcounts.magic.all, file=file.path(infodir, "tcounts.magic.D8_CD42bCD41.Robj" ))
...
write.table(data.frame(PRMT1 = sort(scc["PRMT1",], decreasing=T)), file=file.path(infodir, "spearman.correlation.D8_CD42bCD41.after_magic.PRMT1.txt",sep="\\"), quote=FALSE, na="", sep="\t", col.names=F, row.names=T)
...
# 17880 17880
# rank genes by their correlation with PRMT1
write.table(data.frame(PRMT1 = sort(scc.raw["PRMT1",], decreasing=T)), file=file.path(infodir, "spearman.correlation.D8_CD42bCD41.before_magic.PRMT1.txt",sep="\\"), quote=FALSE, na="", sep="\t", col.names=F, row.names=T)
...
for (cdt in c("D0_unstimulated", "D8_other", "D8_stimulated")){
  ...
  g <- g + geom_vline(xintercept=1000, color="red")
  g <- g + theme_classic()
  ggsave(file.path(figdir, paste("histogram","nCount_RNA","per","cell",cdt,"png", sep=".") ), width=8, height=6, dpi=300)
  ...
  g <- g + scale_color_viridis_c(option="B")
  g <- g + theme_classic()
  ggsave(file.path(figdir,paste("exp","PRMT1","before_magic",cdt,"png",sep="."),sep="\\"), width=8, height=6, dpi=300)
  # after
  g <- ggplot(tcounts.magic) + geom_point(aes(x=VIM, y=GAPDH, color=PRMT1))
  g <- g + scale_color_viridis_c(option="B")
  g <- g + theme_classic()
  ggsave(file.path(figdir,paste("exp","PRMT1","after_magic",cdt,"png",sep="."),sep="\\"), width=8, height=6, dpi=300)
  ...
  # 10656  1077
  write.table(texp.magic, file=gzfile(file.path(infodir,paste("exp","sqrt","after_magic",cdt,"tsv","gz",sep="."),sep="\\")), quote=FALSE, na="", sep="\t", col.names=NA)
  ###write.table(t(as.matrix((counts.magic.all$result)^2)), file=gzfile(paste(infodir,"exp.pseudo_counts.after_magic.tsv.gz",sep="/")), quote=FALSE, na="", sep="\t", col.names=NA)
  ...
  # save magic object
  save(tcounts.magic.all, file=file.path(infodir, paste("tcounts","magic","all",cdt,"Robj",sep=".") ))
}
...
g <- g + theme(axis.title=element_blank(), axis.text=element_blank(), axis.line=element_blank(), axis.ticks=element_blank())
ggsave(file.path(figdir,"SPRING.Plot.use_paper_codes.ggplot.png",sep="\\"), width=10, height=8, dpi=600)
ggsave(file.path(figdir,"SPRING.Plot.use_paper_codes.ggplot.svg",sep="\\"), width=10, height=8)
...
g <- g + theme(legend.position="none")
g <- g + theme(axis.title=element_blank(), axis.text=element_blank(), axis.line=element_blank(), axis.ticks=element_blank())
ggsave(file.path(figdir,"SPRING.Plot.use_paper_codes.ggplot.separated.png",sep="\\"), width=20, height=16, dpi=600)
ggsave(file.path(figdir,"SPRING.Plot.use_paper_codes.ggplot.separated.svg",sep="\\"), width=20, height=16)
